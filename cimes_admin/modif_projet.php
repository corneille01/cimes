...
<meta name="description" content="Formulaire de modification d'un projet de recherche CIMES">
...
<body>
    ...
    <div class="form-wrapper">
        <h1 class="form-page-title">Modification du projet</h1>
        <p class="form-page-sub">Modifiez les champs puis enregistrez.</p>
        ...
        <div class="form-card">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="titre" placeholder="Titre complet du projet">
                </div>
                <div class="form-group">
            ...
            <div class="form-row">
                ...
                <div> <!-- colonne de droite : Coordonnateur + Structure -->
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <label for="site_web_porteur">Site web du porteur du projet</label>
        ...
        <div class="form-actions" style="display:flex; justify-content:flex-end; gap:12px; margin-top:8px;">
            ...
            <a href="#" class="btn-submit" id="envoyer" style="opacity:0.5; pointer-events:none;">
    ...
    <script>
        ...
        function buildLocalisationsFromJSON(jsonStr) {
            try {
                const locs = JSON.parse(jsonStr);
                if (Array.isArray(locs)) {
                    locs.forEach(loc => createLocalisationRow(loc.nom || '', loc.lat || '', loc.lng || ''));
                }
            } catch (e) {
                console.warn('Erreur parse localisations', e);
            }
            if (container.children.length === 0) createLocalisationRow();
            synchroniserMassifs();
        }

        const id = <?php echo $id_projet; ?>;
        let lien = 'modif_projet';
        const btnEnvoyer = document.getElementById('envoyer');

        fetch(`../cimes_api/index_api.php?query=projet&id=${id}`)
            .then(r => r.json())
            .then(data => {
                const projet = data[0];
                if (!projet) {
                    document.getElementById('erreur').textContent = 'Projet introuvable.';
                    document.getElementById('erreur').className = 'alerte error';
                    return;
                }
                document.getElementById('titre').value = projet.titre || '';
                document.getElementById('acronyme').value = projet.acronyme || '';
                document.getElementById('financeur').value = projet.financeur || '';
                document.getElementById('porteur_principal').value = projet.porteur_principal || '';
                document.getElementById('structure_rattachement').value = projet.structure_rattachement || '';
                document.getElementById('partenaires').value = projet.partenaires || '';
                document.getElementById('disciplines').value = projet.disciplines || '';
                document.getElementById('massif').value = projet.massif || '';
                document.getElementById('pays').value = projet.pays || '';
                document.getElementById('objectif_principal').value = projet.objectif_principal || '';
                document.getElementById('date_debut').value = projet.date_debut || '';
                document.getElementById('date_fin').value = projet.date_fin || '';
                document.getElementById('site_web').value = projet.site_web || '';
                document.getElementById('mots_cles').value = projet.mots_cles || '';
                document.getElementById('site_web_porteur').value = projet.site_web_porteur || '';

                container.innerHTML = '';
                localisationCounter = 0;
                buildLocalisationsFromJSON(projet.localisations || '[]');
                dataLoaded = true;
                btnEnvoyer.style.opacity = '1';
                btnEnvoyer.style.pointerEvents = 'auto';
            })
            .catch(err => {
                console.error(err);
                document.getElementById('erreur').textContent = 'Erreur lors du chargement des données.';
                document.getElementById('erreur').className = 'alerte error';
                if (container.children.length === 0) createLocalisationRow();
            });

        // L'event listener sur imageFileInput a été supprimé car il n'existe pas dans cette page
        // (il provoquait une erreur dans la console)

        window.getFormData = function() {
            if (!dataLoaded) return Promise.reject('Les données du projet ne sont pas encore chargées.');
            const localisations = [];
    ...
    <script src="js/creer_modif_projet.js" async defer></script>
</body>
