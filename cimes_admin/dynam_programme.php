...
<body>
    <div class="dynam-wrapper">
        <div class="dynam-toolbar">
            ...
            <a href="ajout_programme.php" class="bouton_ajouter"><i class="fas fa-plus"></i> Ajouter</a>
        </div>
    ...
    <script>
        ...
        function chargerTable() {
            ...
            fetch(`${API_URL}?query=programme`)
                .then(r => r.json())
                .then(data => {
                    if (!Array.isArray(data)) {
                        tbody.innerHTML = '<tr><td colspan="4">Erreur format données</td></tr>';
                        return;
                    }
                    if (data.length === 0) {
                        tbody.innerHTML = '<tr><td colspan="4">Aucun programme</td></tr>';
                        return;
                    }
                    let html = '';
                    data.forEach(item => {
                        html += `
                            <tr>
                                ...
                                <td><a href="modif_programme.php?id=${item.id}" class="editer-lien"><i class="fas fa-pen"></i></a></td>
