/* ─────────────────────────────────────────────
   creer_modif_axes.js
   Gère la création (lien = 'cree_axes')
   et la modification (lien = 'modif_axes')
   ───────────────────────────────────────────── */
'use strict';

const estModif = (lien === 'modif_axes');
...
if (!estModif) {
    document.title = 'Ajouter un axe de recherche';
}
...
function envoi_donnees() {
    ...
    const fileInput = document.getElementById('image_axe');
    const file = fileInput?.files[0];
     
    if (!file) {
        afficherErreur('L\'image est obligatoire. Veuillez sélectionner une image pour l\'axe.');
        return;
    }
    ...
    const resetFormulaire = () => {
        [
            'titre_axe', 'description_axe', 'mots_cles_axe'
        ].forEach(fieldId => {
            const el = document.getElementById(fieldId);
            if (el) el.value = '';
        });
        const imageInput = document.getElementById('image_axe');
        ...
        .then(response => {
            ...
            if (r === 'ok') {
                ...
                if (estModif) {
                    ...
                    errEl.innerHTML = `
                        <span>L'axe a bien été enregistré.</span>
                        <div class="alerte-actions">
                            <a href="javascript:history.back();" class="alerte-btn alerte-btn--outline">
                                <i class="fa-solid fa-arrow-left"></i> Retour aux axes
                            </a>
                            <a href="ajout_axes.php" class="alerte-btn alerte-btn--primary">
                                <i class="fa-solid fa-plus"></i> Ajouter un autre axe
                            </a>
                        </div>`;
                }

            } else if (r === 'ko_doublon') {
                const titre = document.getElementById('titre_axe').value.trim();
                afficherErreur(`Un axe nommé "${titre}" existe déjà. Veuillez modifier le titre.`);
