/* ─────────────────────────────────────────────
   creer_modif_actu.js
   Gère la création (lien = 'cree_actu')
   et la modification (lien = 'modif_actu')
   ───────────────────────────────────────────── */
'use strict';

const estModif = (typeof lien !== 'undefined' && lien === 'modif_actu');
...
if (!estModif) {
    document.title = 'Ajouter une actualité';
}
...
function envoi_donnees() {
    ...
    if (typeof window.getFormData === 'function') {
        ...
        if (estModif && typeof idActu !== 'undefined') {
            formData.id = idActu;
        }
    }
    ...
    const fileInput = document.getElementById('image_actu');
    ...
    const resetFormulaire = () => {
        [
            'titre_actu', 'date_actu', 'lieu_actu',
            'description_courte_actu', 'description_longue_actu'
        ].forEach(fieldId => {
            const el = document.getElementById(fieldId);
            if (el) el.value = '';
        });
        const imgInput = document.getElementById('image_actu');
        ...
        .then(response => {
            ...
            if (r === 'ok') {
                ...
                if (estModif) {
                    ...
                    errEl.innerHTML = `
                        <span>L'actualité a bien été créée.</span>
                        <div class="alerte-actions">
                            <a href="javascript:history.back();" class="alerte-btn alerte-btn--outline">
                                <i class="fa-solid fa-arrow-left"></i> Retour aux actualités
                            </a>
                            <a href="ajout_actu.php" class="alerte-btn alerte-btn--primary">
                                <i class="fa-solid fa-plus"></i> Ajouter une autre actualité
                            </a>
                        </div>`;
                }
            } else if (r === 'ko_mime') {
