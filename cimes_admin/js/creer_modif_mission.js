/* ─────────────────────────────────────────────
   creer_modif_mission.js
   Gère la création (lien = 'cree_mission')
   et la modification (lien = 'modif_mission')
   ───────────────────────────────────────────── */
'use strict';

const estModif = (lien === 'modif_mission');
...
if (!estModif) {
    document.title = 'Ajouter une mission';
}
...
function envoi_donnees() {
    ...
    /* Validation minimale */
    if (!formData.nom || !formData.texte) {
        afficherErreur('Le nom et le texte de la mission sont obligatoires.');
        return;
    }

    /* Vérification poids image côté client */
    const fileInput = document.getElementById('image_mission');
    ...
    const resetFormulaire = () => {
        ['nom_mission', 'texte_mission'].forEach(fieldId => {
            const el = document.getElementById(fieldId);
            if (el) el.value = '';
        });
        const imageInput = document.getElementById('image_mission');
        ...
        .then(response => {
            ...
            if (r === 'ok') {
                ...
                if (estModif) {
                    ...
                    errEl.innerHTML = `
                        <span>La mission a bien été enregistrée.</span>
                        <div class="alerte-actions">
                            <a href="javascript:history.back();" class="alerte-btn alerte-btn--outline">
                                <i class="fa-solid fa-arrow-left"></i> Retour aux missions
                            </a>
                            <a href="ajout_mission.php" class="alerte-btn alerte-btn--primary">
                                <i class="fa-solid fa-plus"></i> Ajouter une autre mission
                            </a>
                        </div>`;
                }

            } else if (r === 'ko_doublon') {
                const nom = document.getElementById('nom_mission').value.trim();
                afficherErreur(`Une mission nommée "${nom}" existe déjà. Veuillez modifier le nom.`);
