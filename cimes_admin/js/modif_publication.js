...
document.querySelector("#formPub").addEventListener('submit', function(event) {
    ...
    if (inputfile.files.length > 0) {
        if (inputfile.files[0].type == "image/jpeg" || inputfile.files[0].type == "image/jpg" || inputfile.files[0].type == "image/png") {
            ...
            envoi_image(thematique, titre, auteur, date, texte, image);
        } else {
...
function envoi_donnees(thematique, titre, auteur, date, texte, image ) {
    let data = {
        lien: lien,
    ...
    .then(data => {
        ...
        if (data === "ok") {
            document.querySelector("#erreur").innerHTML = `
                ...
                <a href="data_publication.php" class="bouton_ajouter">Revenir à la page de gestion des publications</a>
            `;
        } else {
...
function envoi_image(thematique, titre, auteur, date, texte, image ) {
    ...
    .then(data => {
        if (data === "ok") {
            envoi_donnees(thematique, titre, auteur, date, texte, image );
