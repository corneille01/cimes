...
<title>Modification d'un axe de recherche</title>
<meta name="description" content="Formulaire de modification d'un axe de recherche">
...
<body>
    ...
    <div class="form-wrapper">
        <h1 class="form-page-title">Modification d'un axe de recherche</h1>
    ...
    <?php $id_axe = intval($_GET['id'] ?? $_GET['quelaxe'] ?? 0); ?>
    <script>
        const id = <?php echo $id_axe; ?>;
        let lien = 'modif_axes';
        ...
        /* ── Données du formulaire exposées pour creer_modif_axes.js ── */
        ...
        const CHAMPS = [
            ['#titre_axe', 'titre'],
            ['#description_axe', 'description'],
            ['#mots_cles_axe', 'mots_cles'],
        ];
        ...
        fetch(`../cimes_api/index_api.php?query=axes&id=${id}`)
            .then(r => {
            ...
            .then(data => {
                ...
                CHAMPS.forEach(([sel, cle]) => {
                    const el = document.querySelector(sel);
                    if (el && axe[cle] != null) el.value = axe[cle];
                });

                /* Image existante */
                if (axe.image) {
                    const src = axe.image.startsWith('http') ? axe.image : `img/${axe.image}`;
                    document.getElementById('image-preview').innerHTML =
                ...
                /* Titre dynamique */
                if (axe.titre) {
                    document.title = `Modifier – ${axe.titre}`;
                    const h1 = document.querySelector('.form-page-title');
                    if (h1) h1.textContent = `Modification de l'axe : ${axe.titre}`;
                }
            })
            .catch(err => {
                CHAMPS.forEach(([sel]) => {
                ...
                console.error('Erreur chargement axe :', err);
                const errEl = document.querySelector('#erreur');
                if (errEl) {
                    errEl.className = 'alerte error';
                    errEl.textContent = 'Impossible de charger les données de l\'axe.';
                }
            });
    </script>
