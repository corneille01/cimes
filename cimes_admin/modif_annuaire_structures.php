...
<title>Modification d'une structure – Annuaire</title>
<meta name="description" content="Formulaire de modification d'une structure de l'annuaire">
...
<body>
    ...
    <div class="form-wrapper">
        <h1 class="form-page-title">Modification d'une structure</h1>
    ...
    <?php $id_structure = intval($_GET['id'] ?? $_GET['quelstructure'] ?? 0); ?>
    <script>
        const id = <?php echo $id_structure; ?>;
        let lien = 'modif_structure';
        ...
        /* Données du formulaire pour creer_modif_annuaire_structures.js
           zones_intervention retiré — colonne inexistante en base */
        ...
        /* Champs à pré-remplir — zones_intervention retiré */
        const CHAMPS = [
            ['#etablissement_annuaire', 'etablissement'],
            ['#responsable_annuaire', 'responsable'],
            ['#discipline_annuaire', 'discipline'],
            ['#domaine_recherche_annuaire', 'domaine_recherche'],
            ['#tutelles_annuaire', 'tutelles'],
            ['#annee_creation_annuaire', 'annee_creation'],
            ['#adresse_annuaire', 'adresse'],
            ['#site_web_annuaire', 'site_web'],
            ['#presentation_annuaire', 'presentation'],
        ];
        ...
        fetch(`../cimes_api/index_api.php?query=annuaire_structure&id=${id}`)
            .then(r => {
            ...
            .then(data => {
                ...
                if (!structure) {
                    console.warn('Aucune donnée pour id =', id);
                    return;
                }

                CHAMPS.forEach(([sel, cle]) => {
                    const el = document.querySelector(sel);
                    if (el && structure[cle] != null) el.value = structure[cle];
                });

                if (structure.photo) {
                    document.getElementById('photo-preview').innerHTML =
                        `<img src="../cimes_clients/img/${structure.photo}" alt="Logo actuel">`;
                }

                const nom = structure.etablissement ?? '';
                if (nom) {
                    document.title = `Modifier – ${nom}`;
                    const h1 = document.querySelector('.form-page-title');
                    if (h1) h1.textContent = `Modification de ${nom}`;
                }
            })
            .catch(err => {
                CHAMPS.forEach(([sel]) => {
                ...
                if (errEl) {
                    ...
                    errEl.textContent = 'Impossible de charger les données de la structure.';
                }
            });
    </script>
